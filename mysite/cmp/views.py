...
def commodity_cmp_make(request, category_id, cmmodity_id, cmp_lists=None, cmp_lists_idx=None, cmp_cmmodity_id=None):
    '''商品比較用のページ作成'''
    ...
    if cmp_lists is None:
        cmp_lists = category.cmp_lists_make(cmmodity_id)
    ...
    #対象商品と比較対象商品を取得
    commodity_A = get_object_or_404(Commodity, id=cmmodity_id) #対象商品を取得
