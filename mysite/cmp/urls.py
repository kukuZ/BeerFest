...
urlpatterns = [
    ...
    url(r'^cmpare/(?P<category_id>\d+)/(?P<commodity_id>\d+)/$', views.commodity_cmp_make, name='commodity_cmp_make'),        # 登録
    url(r'^cmpare_post/(?P<commodity_A_id>\d+)/(?P<commodity_B_id>\d+)/(?P<attributelist_id>\d+)/(?P<skip>\w+)/(?P<cannot>\w+)/$', views.commodity_cmp_post, name='commodity_cmp_post'),        # 登録
