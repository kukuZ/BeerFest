...
urlpatterns = [
    ...
    url(r'^cmpare_post/(?P<commodity_A_id>\d+)/(?P<commodity_B_id>\d+)/(?P<attributelist_id>\d+)/(?P<vote>\d+)/(?P<cmp_lists>\d+)/(?P<pre_cmp_lists_idx>\d+)/$', views.commodity_cmp_post, name='commodity_cmp_post'),        # 登録
